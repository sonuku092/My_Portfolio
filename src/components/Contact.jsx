...
const Contact = () => {
  ...
  const isNameValid =
    !nameTouched || (name.trim().length > 0 && name.length <= maxNameChars);
  const isEmailValid =
    !emailTouched || (email.trim().length > 0 && email.length <= maxEmailChars);
    ...
    >
      <div className="flex-1 flex flex-col rounded-lg text-white">
        ...
        <form ref={form} onSubmit={sendEmail} className="space-y-4">
          <div className="flex-row space-y-2 sm:space-y-0 sm:flex gap-4">
            <div className="flex-1">
              <label
                htmlFor="name"
                className={`block text-sm px-2 font-medium ${
                  nameTouched && !isNameValid ? "text-red-500" : "text-gray-300"
                }`}
              >
                Name
              </label>
          ...
          <button
            type="submit"
            className=" w-full sm:w-fit inline-flex items-center text-center px-4 py-2 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
            Send
          </button>
        </form>
